...
namespace MidnightCommander
{
    class FileBrowserWindow : Window
    {
        ...
        public FileBrowserWindow(Application parentApplication)
        {
        ...
        {
            if (DialogActive && info.Key == ConsoleKey.Escape)
                return;
            ...
            else if(info.Key == ConsoleKey.F4)
            {
                foreach (Table t in tables)
                {
                    if (t.IsActive)
                    {
                        if(t.selectedFile.Contains(".txt"))
                            ParentApplication.ActiveWindow = new FileEditor(Path, ParentApplication);
                    }
                }
            }    
            ...
            else if (info.Key == ConsoleKey.F9)
            {
                ...
                */
                ...
                if (ColorTransfer.index == 3)
                    ColorTransfer.index = 0;
                else
                    ColorTransfer.index++;
                Refresh();
            }
